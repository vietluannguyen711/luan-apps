package com.example.quizcartservice.controller;

import com.example.quizcartservice.service.CartService;
...
public class CartController {
    ...
    @PostMapping("/{userId}")
    public ResponseEntity<?> addNewCartItem(
    ...
    public ResponseEntity<?> updateItemAmount(
        ...
        try {
            return ResponseEntity.ok(
                    cartService.updateAmount(userId, cartItemId, amount));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    }
